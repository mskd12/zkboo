...
void mpc_AND_u8(uint8_t x[3], uint8_t y[3], uint8_t z[3], unsigned char *randomness[3], int* randCount, View views[3], int* countY) {
	
	uint8_t r[3] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount), getRandom8(randomness[2], *randCount)};
	*randCount += 1;
	uint8_t t[3] = {0,0,0};
...
void mpc_ANDK_u8(uint8_t x[3], uint8_t y, uint8_t z[3], unsigned char *randomness[3], int* randCount, View views[3], int* countY) {
	uint8_t r[3] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount), getRandom8(randomness[2], *randCount)};
	*randCount += 1;
	uint8_t t[3] = {0,0,0};
	...
	if (*countY == 6896) {
		printf("debug\n");
		printf("%d, %d\n", t[0] ^ t[1] ^ t[2],  (x[0] & y) ^ (x[1] & y) ^ (x[2] & y));
		printf("%d, %d, %d, %d, %d, %d, %d\n", x[0], x[1],x[2], y, r[0], r[1], r[2]);
		printf("%d, %d, %d, %d\n", *countY, views[0].y[*countY], *randCount,r[0]);
		printf("%d, %d, %d, %d\n", *countY, views[1].y[*countY], *randCount,r[1]);
		printf("%d, %d, %d, %d\n", *countY, views[2].y[*countY],*randCount,  r[2]);
...
int mpc_ANDK_verify_u8(uint8_t x[2], uint8_t y, uint8_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	uint8_t r[2] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount) };
	*randCount += 1;
	...
	if(ve->y[*countY] != t) {
		...
		// printf("%d, %d, %d, %d, %d\n", *countY, ve1->y[*countY], t, *randCount, t1 );
		// for (int i=0; i < 8; i++){
		// 	printf("%d",  GETBIT(t, i));
		// }
		// printf("\n");
...
int mpc_AND_verify_u8(uint8_t x[2], uint8_t y[2], uint8_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	uint8_t r[2] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount) };
	*randCount += 1;
...
void mpc_OR_u8(uint8_t x[3], uint8_t y[3], uint8_t z[3], unsigned char *randomness[3], int* randCount, View views[3], int* countY){
...
void mpc_ADD(uint32_t x[3], uint32_t y[3], uint32_t z[3], unsigned char *randomness[3], int* randCount, View views[3], int* countY) {
	...
	uint32_t r[3] = { getRandom32(randomness[0], *randCount), getRandom32(randomness[1], *randCount), getRandom32(randomness[2], *randCount)};
	*randCount += 4;
	// printf("mpc_ADD: %d, %d, %d, %d, %d\n", *randCount, r[0], r[1], r[2], *countY);
...
void mpc_ADD_u8(uint8_t x[3], uint8_t y[3], uint8_t z[3], unsigned char *randomness[3], int* randCount, View views[3], int* countY) {
	uint8_t c[3] = { 0 };
	uint8_t r[3] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount), getRandom8(randomness[2], *randCount)};
	*randCount += 1;
	//printf("%d, %d, %d\n", r[0], r[1], r[2]);
...
int mpc_ADD_verify(uint32_t x[2], uint32_t y[2], uint32_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	...
	uint32_t r[2] = { getRandom32(randomness[0], *randCount), getRandom32(randomness[1], *randCount) };
	*randCount += 4;
	// printf("mpc_ADD verify: %d, %d, %d, %d\n", *randCount, r[0], r[1], *countY);
	uint8_t a[2], b[2];
...
int mpc_ADD_verify_u8(uint8_t x[2], uint8_t y[2], uint8_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	uint8_t r[2] = { getRandom8(randomness[0], *randCount), getRandom8(randomness[1], *randCount) };
	*randCount += 1;
	//printf("%d, %d\n", r[0], r[1]);
	uint8_t a[2], b[2];
...
int mpc_ADDK_verify(uint32_t x[2], uint32_t y, uint32_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	uint32_t r[2] = { getRandom32(randomness[0], *randCount), getRandom32(randomness[1], *randCount) };
	*randCount += 4;
...
int mpc_ADD_verify_test(uint32_t x[2], uint32_t y[2], uint32_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	clock_t	t_begin = clock();
	uint32_t r[2] = { getRandom32(randomness[0], *randCount), getRandom32(randomness[1], *randCount) };
	*randCount += 4;
...
int mpc_MAJ_verify_u8(uint8_t a[2], uint8_t b[2], uint8_t c[2], uint8_t z[3], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {
	uint8_t t0[2];
	uint8_t t1[2];

	mpc_XOR2_u8(a, b, t0);
	mpc_XOR2_u8(a, c, t1);
	if(mpc_AND_verify_u8(t0, t1, z, ve, ve1, randomness, randCount, countY) == 1) {
		#if VERFDEBUG
		mpc_XOR2_u8(z, a, z);
		#endif
		return FAIL;
...
int mpc_CH_verify_u8(uint8_t e[2], uint8_t f[2], uint8_t g[2], uint8_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY) {

	uint8_t t0[2];
	mpc_XOR2_u8(f,g,t0);
	if(mpc_AND_verify_u8(e, t0, t0, ve, ve1, randomness, randCount, countY) == 1) {
		#if VERFDEBUG
		mpc_XOR2_u8(t0,g,z);
		#endif
		return FAIL;
...
int mpc_OR_verify_u8(uint8_t x[2], uint8_t y[2], uint8_t z[2], View* ve, View* ve1, unsigned char* randomness[2], int* randCount, int* countY){
